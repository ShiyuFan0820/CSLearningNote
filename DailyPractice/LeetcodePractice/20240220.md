...
```
_**Failed Analysis： Python in leetcode may not support list comprehension.**_
...
class Solution:
    def removeElement(self, nums: List[int], val: int) -> int:
        ...
        for ind in range(len(nums)):
            replace_ind = -1
            if nums[ind] == val:
                while nums[replace_ind] == val:
                    replace_ind -= 1
                if ind - replace_ind == len(nums) + 1:
                    k = ind
                    return k
                nums[ind] = nums[replace_ind]
